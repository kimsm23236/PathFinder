...
public class PathFinder : GSingleton<PathFinder>
{
    ...
    #endregion // A* 알고리즘으로 최단거리를 찾기 위한 변수
    protected override void Init()
    {
        mapBoard = GFunc.GetRootObj("GameObjs").FindChildComponent<MapBoard>("MapBoard");
    }
    ...
    private IEnumerator DelayFindPath_Astar(float delay_)
    {
        ...
        GFunc.Log($"sonp : {sourceObjNameParts[0]}");
        int sourceIdx1D = -1;
        ...
        while (isFoundDestination == false && isNowayToGo == false)
        {
            ...
            GFunc.Log($"open count : {aStarOpenPath.Count}");
            GFunc.Log($"close count : {aStarClosePath.Count}");
            ...
            else
            {
                ...
                foreach(var nextIdx1D in nextSearchIdx1Ds)
                {
                    ...
                    if (aStarClosePath.Contains(nextNode)) { GFunc.Log("nextNode in ClosePath"); }
    ...
    private void Add_AstarOpenList(AstarNode targetTerrain_, AstarNode prevNode = default)
    {
        ...
        else
        {
            ...
            if(openedNode != default && closeNode != null)
            {
    ...
    private void Update_AstarCostToTerrain(AstarNode targetNode, AstarNode prevNode)
    {
        ...
        targetNode.UpdateCost(totalDistance2D, heuristic, prevNode);
        // } 이전 노드가 존재하는 경우 이전 노드의 코스트를 추가해서 연산한다.
